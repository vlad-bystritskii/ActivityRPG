...
public struct HomeFeature {
    ...
    public struct State: Equatable {
        ...
        var week: [Date] = Date().weekStrip        // 7 дат, начиная с понедельника текущей недели
    ...
    public var body: some Reducer<State, Action> {
        Reduce { state, action in
            ...
            case .onAppear, .reloadRequested:
                state.week = state.selectedDate.weekStrip // синхронизируем полосу
                return .run { [container, date = state.selectedDate] send in
                    let items: [WorkoutDTO] = try await MainActor.run {
                        ...
                        return try ctx.fetch(d).map { .init(id: $0.id, date: $0.date, type: $0.type, name: $0.name) }
                    }
                    await send(.workoutsLoaded(items))
                }
                
            case let .dayTapped(d):
                state.selectedDate = d.startOfDay
                state.week = d.weekStrip
